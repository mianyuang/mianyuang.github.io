...
`另一个解决的思路` ：我们只是想让已经提交了的事务对数据库中数据所做的修改永久生效，即使后来系统崩溃，在重启后也能把这种修改恢复出来。所以我们其实没有必要在每次事务提交时就把该事务在内存中修改过的全部页面刷新到磁盘，只需要把修改了哪些东西记录一下 就好。比如，某个事务将系统 表空间中第10号 页面中偏移量为 100 处的那个字节的值 1 改成 2 。我们只需要记录一下：将第0号表 空间的10号页面的偏移量为100处的值更新为 2
...
![](image_zfNyCFF0u7.png)

### redo的整体流程

以一个更新事务为例，redo log 流转过程，如下图所示：

![](image_ycyykQTbTn.png)
...
![](image_2yYzF6y4b6.png)

### 从数据操作的类型划分：读锁、写锁

![](image_DBAmc3J0Yt.png)
...
![](image_uP0XIpJRCI.png)

#### 锁定读

![](image_DP_aG0Ca9g.png)

![](image_oqE3GZqk_U.png)

![](image_tXsj7AqCEU.png)
...
![](image_9-B0D9jS5W.png)

#### 表锁（Table Lock）

![](image_-MuJvP-ceS.png)
...
![](image_-kKTb6fTQ5.png)

或者

![](image_BrhOCTSR7W.png)
...
![](image_kq_k_lySl1.png)

步骤5：加读锁

我们为mylock表加read锁（读阻塞写），观察阻塞的情况，流程如下：

![](image_sW38ykrMsy.png)

![](image_cdzTa1AdId.png)

步骤6：加写锁

为mylock表加write锁，观察阻塞的情况，流程如下：

![](image_MH58YjdmLr.png)

![](image_YCH0p9CbsD.png)
...
![](image_P5SJgaWvkR.png)

![](image_x3g9bUWwTC.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713161549241.png)

student表中的聚簇索引的简图如下所示。

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713163353648.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713164811567.png)

举例如下：

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713164948405.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713171650888.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713174726264.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713174108634.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713174551814.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713174602102.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713175032619.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713192418730.png)

**③ 临键锁（Next-Key Locks）**

有时候我们既想 `锁住某条记录` ，又想 阻止 其他事务在该记录前边的 间隙插入新记录 ，所以InnoDB就提 出了一种称之为 Next-Key Locks 的锁，官方的类型名称为： LOCK\_ORDINARY ，我们也可以简称为 next-key锁 。Next-Key Locks是在存储引擎 innodb 、事务级别在 可重复读 的情况下使用的数据库锁， innodb默认的锁就是Next-Key locks。比如，我们把id值为8的那条记录加一个next-key锁的示意图如下：

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713192549340.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713203124889.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713203532124.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713203619704.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713203714577.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713204544767.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713205010502.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713205135694.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713210951100.png)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713211139670.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713211417909.png)

### 按加锁的方式划分：显式锁、隐式锁

#### 隐式锁

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713211525845.png)

-   **情景一**：对于聚簇索引记录来说，有一个 `trx_id` 隐藏列，该隐藏列记录着最后改动该记录的 `事务 id` 。那么如果在当前事务中新插入一条聚簇索引记录后，该记录的 trx\_id 隐藏列代表的的就是 当前事务的 事务id ，如果其他事务此时想对该记录添加 S锁 或者 X锁 时，首先会看一下该记录的 trx\_id 隐藏列代表的事务是否是当前的活跃事务，如果是的话，那么就帮助当前事务创建一个 X 锁 （也就是为当前事务创建一个锁结构， is\_waiting 属性是 false ），然后自己进入等待状态 （也就是为自己也创建一个锁结构， is\_waiting 属性是 true ）。
-   **情景二**：对于二级索引记录来说，本身并没有 trx\_id 隐藏列，但是在二级索引页面的 Page Header 部分有一个 `PAGE_MAX_TRX_ID` 属性，该属性代表对该页面做改动的最大的 `事务id` ，如 果 PAGE\_MAX\_TRX\_ID 属性值小于当前最小的活跃 事务id ，那么说明对该页面做修改的事务都已 经提交了，否则就需要在页面中定位到对应的二级索引记录，然后回表找到它对应的聚簇索引记 录，然后再重复 情景一 的做法。

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713214522709.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713220714098.png)

**举例2：**

用户A给用户B转账100，再次同时，用户B也给用户A转账100。这个过程，可能导致死锁。

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713220936236.png)
...
**方式1：** 等待，直到超时（innodb\_lock\_wait\_timeout=50s)

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713221418100.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713221758941.png)

基于这两个信息，可以绘制wait-for graph（等待图）

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220713221830455.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220714131008260.png)
...
![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220714132306208.png)
...
-   行锁：
    ...
    > n\_bits的值一般都比页面中记录条数多一些。主要是为了之后在页面中插入了新记录后 也不至于重新分配锁结构

`4. type_mode` ：

这是一个32位的数，被分成了 `lock_mode` 、 `lock_type` 和 `rec_lock_type` 三个部分，如图所示：

![](https://github.com/codinglin/StudyNotes/raw/main/MySQL高级篇/MySQL事物篇.assets/image-20220714133319666.png)
...
在InnoDB存储引擎中，LOCK\_IS，LOCK\_IX，LOCK\_AUTO\_INC都算是表级锁的模式，LOCK\_S和 LOCK\_X既可以算是表级锁的模式，也可以是行级锁的模式。
...
MVCC在MySQL InnoDB中的实现主要是为了提高数据库并发性能，用更好的方式去处理 `读-写冲突` ，做到 即使有读写冲突时，也能做到 `不加锁` ， `非阻塞并发读` ，而这个读指的就是 `快照读` , 而非 `当前读` 。当前读实际上是一种加锁的操作，是悲观锁的实现。而MVCC本质是采用乐观锁思想的一种方式。
...
![](image-20220714140441064.png)

![](image-20220714140510426.png)
...
![](image-20220715130916437.png)

## 举例说明

![](image-20220715131200077.png)
...
![](image-20220715135017000.png)

![](image-20220715135143939.png)
...
![](image-20220715140555172.png)

![](image-20220715140620328.png)
